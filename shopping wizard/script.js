...
-- 10. FUNCION PARA VOLVER AL PRINCIPIO (set), se encarga de:
...
function setColor(iColorsH, iColors) {
  ...
  //PARTE DEL LEGEND EN DISABLED---------------
  //estudiar para que se enseñe cuando se ponga en enabled
  const colorLegend = document.querySelector(".color-legend");
...
function checkProfileValidity() {
  //let errorUser = window.getComputedStyle(document.querySelector(".username-fieldset"), '::before');
  //let errorEmail = window.getComputedStyle(document.querySelector(".username-fieldset"), '::after');
  //let errorPassword = window.getComputedStyle(document.querySelector(".password-fieldset"), '::after');
  const error = document.querySelector(".username-fieldset");
  const errorP = document.querySelector(".password-fieldset");
...
function setAddress() {
  ...
  //selectPrefix.value = "49";  TO SET ONE AFTER WHICH COUNTRY HAS BEEN ENTERED
}
